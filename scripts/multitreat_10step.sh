...
HYDRA_FULL_ERROR=1 python src/multitreat.py experiment=multitreat_10step logger=wandb +logger.wandb.name=multitreat_10step_lr_longer ++ckpt_paths="[$ckpt_paths]" ++treat.learning_rate=0.01 ++treat.n_iter=50

# Effect of the clamp
HYDRA_FULL_ERROR=1 python src/multitreat.py experiment=multitreat_10step logger=wandb +logger.wandb.name=multitreat_10step_clampslope_low ++ckpt_paths="[$ckpt_paths]" ++treat.clamp_slope=0.001
HYDRA_FULL_ERROR=1 python src/multitreat.py experiment=multitreat_10step logger=wandb +logger.wandb.name=multitreat_10step_clampmax_low ++ckpt_paths="[$ckpt_paths]" ++treat.clamp_max=4.0 ++treat.learning_rate=0.025
